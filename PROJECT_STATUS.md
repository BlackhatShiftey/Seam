...
Last updated: 2026-05-26 (HISTORY#265 - H2 substrate slice 4 dev/holdout split helper)
...
- Latest continuity handoff is `HISTORY#265` - H2 substrate slice 4 dev/holdout split helper. Slice 4 adds `tools/h2/holdout_split.py`, a deterministic partition primitive that hashes each benchmark `case_id` under a salt + dev ratio into either the `dev` or `holdout` bucket, materialised as a JSON manifest committed to the repo (`schema: seam-holdout-split/v1`, sorted assignment keys). The CLI is `python -m tools.h2.holdout_split --source quickstart|<path> --manifest <path> [--salt SALT] [--ratio R] [--rewrite]`; default salt is `seam-locomo-v1` and default ratio is `0.8`. `update_manifest` is idempotent: re-running with the same salt/ratio leaves existing assignments untouched and appends only new `case_ids`; changing salt or ratio against an existing manifest is rejected unless `--rewrite` is passed. Consumer helpers `dev_case_ids`, `holdout_case_ids`, `is_holdout` are the slice 5 entry points. 20 tests in `tests/audit/test_h2_holdout_split.py` cover determinism, ratio guardrails, manifest round-trip, idempotent append, salt/ratio change rejection, --rewrite recompute, and CLI semantics. Materialising the actual `benchmarks/external/locomo/holdout_assignment.json` against the real LoCoMo dataset remains operator-gated and is not done in this slice. PR #36 (HISTORY#263-#264) has been merged into `main` as commit `eeadf1c`; PR #34 (HISTORY#251-#261) was merged as `c2dd45d`. `HISTORY#250` remains the GitHub PR stack closeout; `HISTORY#246` records the active GitHub ruleset `Protect main (PR + hygiene gates)`, and `HISTORY#249` records the `AGENTS.md` PR workflow for future agents. Step 4 final paid measurement remains operator-gated.
