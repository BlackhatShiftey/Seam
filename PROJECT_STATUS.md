...
Last updated: 2026-05-25 (HISTORY#244 - H2 retrieval_event substrate)
...
- Latest continuity handoff is `HISTORY#244` - H2 substrate slice 1 landed. `seam_runtime/storage.py` adds a canonical append-only `retrieval_event` table with autoincrement `event_id`, fields per `docs/roadmap/CONTEXT_STREAMS.md` section 12.5 (ts, run_id, scope, query, candidate_ids_json, ranks_json, scores_json, reasons_json, context_hash, gold_answer, gold_hit_ids_json, context_recall, judge_score, answer, source_kind, source_ref, stale_source, schema_version, extra_json), and three indexes (run_id, ts, stale_source). `SQLiteStore` now exposes `write_retrieval_event(...)`, `iter_retrieval_events(...)`, and `count_retrieval_events(...)`; the append-only contract is enforced by absence of any update/delete/purge method. `tests/audit/test_retrieval_event_store.py` covers schema presence, minimal+full round-trip, stale-source filtering, run/scope/limit filters, the append-only contract, required-field validation, ranks/scores alignment, and empty-candidate recording (9 tests, all green). No writer hook or backfill yet; the LoCoMo adapter and runner are untouched. `HISTORY#243` remains the roadmap pivot of record. Next slices: opt-in writer hook in `SeamLocomoAdapter.answer()`, BIL-2 backfill tool with stale flagging, and a dev/holdout split helper before any scoring-weight tuning. All ranking-policy promotion still gated by `seam improvement review`. Step 4 final paid measurement remains operator-gated.
