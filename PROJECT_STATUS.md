...
Last updated: 2026-05-22 (HISTORY#232 — Judge Batch API Phase A)
...
- Latest continuity handoff is `HISTORY#232` — Judge Batch API Phase A is landed for external benchmark judges. `benchmarks/external/common/judge.py` now has `JudgeBatchItem` plus opt-in `score_batch` implementations for Claude and OpenAI judges; `benchmarks/external/common/runner.py` can defer primary and cross-judge scoring through a provider batch when `judge_batch=True`; `benchmarks/external/locomo/run.py` exposes `--judge-batch`; and `test_seam_all/test_locomo_judge_batch.py` covers the no-network batch behavior. Phase B answerer batching remains explicitly deferred. `docs/SOP_TRACK_M_P4_SCORE_IMPROVEMENTS_AND_MEASUREMENT.md` (from `HISTORY#228`) remains the planned score-improvement execution; its OpenAI quickstart baseline has been produced outside the repo under `/tmp/seam-track-m/`, while the full 1,542-case paid LoCoMo baseline remains operator-gated. `docs/prompts/DEEPSEEK_TRACK_M_BATCH_JUDGE_ADVISED_PROMPT.md` remains available as the ready-to-paste advised executor prompt. Unrelated local edits in `tests/audit/test_bench_stub_seal_gate.py`, `tests/audit/test_benchmark_endpoint_safety.py`, and `HANDOFF_BATCH_API.md` may be present and should be reviewed separately before staging.
