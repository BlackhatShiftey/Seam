...
Last updated: 2026-05-22 (HISTORY#231 — Ready-to-paste advised DeepSeek prompt)
...
- Latest continuity handoff is `HISTORY#231` — `docs/prompts/DEEPSEEK_TRACK_M_BATCH_JUDGE_ADVISED_PROMPT.md` is a ready-to-paste `claude-ds` prompt with the Track M batch-judge review `ADVISOR_TASK_PACKET` already inserted. The advisor/executor loop protocol is documented in `docs/SOP_ADVISOR_EXECUTOR_LOOP.md`, with the reusable executor prompt in `docs/prompts/DEEPSEEK_ADVISED_EXECUTOR_PROMPT.md` and DeepSeek correction card C6 in `docs/ledgers/agents/deepseek.md`. `docs/SOP_TRACK_M_P4_SCORE_IMPROVEMENTS_AND_MEASUREMENT.md` (from `HISTORY#228`) remains the planned score-improvement execution; its OpenAI quickstart baseline has been produced outside the repo under `/tmp/seam-track-m/`, while the full 1,542-case paid LoCoMo baseline remains operator-gated. Focused 7-file SOP regression test set passed exit 0 to 100% under `.venv/bin/python -m pytest --tb=short -q`; broad `pytest test_seam_all/ tests/audit/` was interrupted at ~13% during the 2026-05-22 audit and remains unverified. Pre-existing local edits in benchmark/judge files may be present and should be reviewed separately before staging.
