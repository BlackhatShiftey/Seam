...
Last updated: 2026-05-22 (HISTORY#233 - Track M P4 temporal-distance scoring)
...
- Latest continuity handoff is `HISTORY#233` - Track M P4 temporal-distance scoring is landed. `seam_runtime/temporal.py` now parses anchored relative question references such as "three weeks ago"; `seam_runtime/retrieval.py` can rank by temporal distance through `temporal_reference` without changing the existing weight formula; `seam_runtime/runtime.py` threads the option through `search_ir`; and `benchmarks/external/locomo/adapters/seam.py` anchors relative LoCoMo questions to the earliest turn timestamp in the scope. No full 1,542-case paid LoCoMo run and no provider judge/answerer calls were made for this slice. `HISTORY#232` remains the Judge Batch API Phase A handoff: Phase B answerer batching is still deferred, and full paid LoCoMo measurement remains operator-gated. `docs/prompts/DEEPSEEK_TRACK_M_BATCH_JUDGE_ADVISED_PROMPT.md` remains available as the ready-to-paste advised executor prompt. Unrelated local edits in `tests/audit/test_bench_stub_seal_gate.py`, `tests/audit/test_benchmark_endpoint_safety.py`, and `HANDOFF_BATCH_API.md` may be present on the primary checkout and should be reviewed separately before staging.
