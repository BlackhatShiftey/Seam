...
Last updated: 2026-05-25 (HISTORY#247 - GitHub maintenance workflow handoff)
...
- Latest continuity handoff is `HISTORY#247` - repo hygiene continued on branch `codex/repo-hygiene-ruleset-record`. `HISTORY#246` records the GitHub main ruleset change: `Protect main (PR + hygiene gates)` applies to `refs/heads/main` with no bypass actors, blocks deletion/non-fast-forward updates, requires PRs, and requires `repo-hygiene`, `chroma-real-smoke`, and `locomo-quickstart-bil2`. The current branch also contains an in-progress scheduled/manual repository-maintenance workflow (`.github/workflows/repository-maintenance.yml`) plus `tools/ci/github_maintenance_report.py` and tests to report open/stale PRs and stale remote branches. `benchmarks/external/locomo/adapters/seam.py` is dirty with H2 retrieval-event writer-hook work that was not touched or staged for the repo-hygiene handoff. Step 4 final paid measurement remains operator-gated.
