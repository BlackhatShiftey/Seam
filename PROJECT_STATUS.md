...
Last updated: 2026-05-26 (HISTORY#264 - PR #36 credential-scope correction)
...
- Latest continuity handoff is `HISTORY#264` - PR #36 credential-scope correction. PR #36 is the repo-maintenance cleanup branch for deleting the dead tracked `experimental/hybrid_orchestrator/` compatibility package, preserving canonical `experimental.retrieval_orchestrator` Hybrid* aliases, removing the stale README compatibility claim, and replacing the legacy import test with a removal guard. Credential scope was corrected after operator clarification: `.env.local`/`.env` are ignored and not tracked; `git log --all --name-only -- .env .env.local '.env.*'` shows only `.env.example`, so there is no GitHub-tracked `.env.local` credential file to remove in PR #36. The prior local `.env.local` deletion was not a PR/GitHub-side change and cannot remove anything from GitHub. The canonical git pre-commit hook was reinstalled with `bash tools/git-hooks/install.sh --force`; the current copy carries the source `CANONICAL_SHA` marker and a normal installer rerun reports `Already installed (copy, sha matches)`. Ignored local `experimental/**/__pycache__/` directories were deleted; the claim that those caches were tracked did not reproduce (`git ls-files 'experimental/**/__pycache__/**'` returned no files). PR #34 (HISTORY#251-#261) has been merged into `main` as commit `c2dd45d`; `HISTORY#262` remains the H2 substrate slice 3 BIL-2 backfill tool handoff. `HISTORY#250` remains the GitHub PR stack closeout; `HISTORY#246` records the active GitHub ruleset `Protect main (PR + hygiene gates)`, and `HISTORY#249` records the `AGENTS.md` PR workflow for future agents. Step 4 final paid measurement remains operator-gated.
