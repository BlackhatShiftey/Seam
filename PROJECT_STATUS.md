...
Last updated: 2026-05-22 (HISTORY#235 - Track M P4 Step 2 reranker)
...
- Latest continuity handoff is `HISTORY#235` - Track M P4 Step 2 cross-encoder reranker is landed after advisor review. `benchmarks/external/locomo/run.py` exposes `--rerank {none,cross-encoder}`; `benchmarks/external/locomo/adapters/seam.py` keeps rerank disabled by default and re-sorts only the top-K candidates when explicitly enabled; `benchmarks/external/locomo/rerank.py` wraps `sentence_transformers.CrossEncoder` with a small model cache and clear install error; and `tests/audit/test_cross_encoder_rerank.py` covers the no-network behavior. No paid API calls, no full LoCoMo run, no ranking weight changes, and no Step 3 embedding-default work were performed in this slice. `HISTORY#234` remains the Step 0a quickstart baseline of record with `gpt-5-mini` answerer at recall=0.9633, EM=0.7000, F1=0.9049, judge_score=0.9500. Step 0b (full 1,542-case LoCoMo with `--answerer-model gpt-5-mini --judge-model gpt-5-nano --judge-batch`) and Step 4 final measurement remain operator-gated paid runs. Phase B answerer batching from HISTORY#232 is still deferred pending Step 0b cost data.
