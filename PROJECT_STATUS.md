...
- Latest continuity handoff is `HISTORY#189` — Track I COMPLETE: SOPs 3+4 (Mem0 + Zep/Graphiti LoCoMo comparators) landed behind `seam[bench-mem0]` and `seam[bench-zep]` optional extras. Full Track I scope (SOPs 0-4) is now merged: registry + CLI (SOP 0), SEAM LoCoMo adapter with string-match scoring + 60s quickstart fixture (SOP 1), optional LLM-judge with stub/claude/openai providers (SOP 2), Mem0 comparator adapter (SOP 3), Zep/Graphiti comparator adapter (SOP 4). Three-way SEAM/Mem0/Zep scoring is reproducible on the quickstart fixture. No SEAM-vs-X public claims pending Track K (BIL bundles). `HISTORY#188` was SOP 2 LLM-judge.
...
- GitHub PR state as of 2026-05-17: PRs #22, #18, #23, #25 (SOP 0), #26, #27 (SOP 1), #28 (SOP 2) merged. SOPs 3+4 (Mem0 + Zep comparators) on branch `feat/track-i-sop3-4-mem0-zep-comparators` pending PR. Track I is complete with this PR merge. PRs #22, #18, and #23 are all merged into main. PR #19 is still draft, conflicting, and must be treated as a partial extraction source because its branch contains private-session-link material in commit metadata. PR #24 (Track I 5-SOP handoff series) was draft and is superseded.
