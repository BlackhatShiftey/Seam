...
- Latest continuity handoff is `HISTORY#171`. **Track H1 Context Streams substrate is fully implemented and measured.** `tools/streams/` provides generic stream tooling; root `HISTORY.md` + `HISTORY_INDEX.md` remain canonical with byte-equivalent derived mirrors under `.seam/streams/history/`; the `roadmap` stream is now populated for every track in `ROADMAP.md` (34 items, not just Track H); `.seam/streams/roadmap/state.md` is the compact status view derived from `seam:item` markers; `.seam/cross_index.md` is the derived global temporal join and now exercises archive rolling (`cross_index_archive/0001-0004.cross.md`). `tools/streams/build_context_pack.py` is the stream-aware pack builder; for `--stream history` it delegates to the canonical history pack for byte-equivalent output. `tools/streams/bloat_report.py` reports measured reductions: **93.5%** for roadmap reads (ROADMAP.md → state.md), **90.5%** for history map reads, **91.0%** for cross-stream recent reads. 11 streams unit tests pass; full verify chain (`verify_integrity`/`verify_routing`/`verify_continuity`/`verify_streams`) is green. Path canonicality flip for `history` remains deferred to a separate later HISTORY entry per CONTEXT_STREAMS.md §9. Snapshot JSON files are local derived artifacts, so a fresh Linux clone should regenerate the resume snapshot with the command in `docs/setup.md` before running continuity verification.
