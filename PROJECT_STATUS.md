...
Last updated: 2026-05-25 (HISTORY#248 - repository maintenance workflow completion)
...
- Latest continuity handoff is `HISTORY#248` - repo hygiene continued on branch `codex/repo-hygiene-ruleset-record` and draft PR #32. `HISTORY#246` records the GitHub main ruleset change: `Protect main (PR + hygiene gates)` applies to `refs/heads/main` with no bypass actors, blocks deletion/non-fast-forward updates, requires PRs, and requires `repo-hygiene`, `chroma-real-smoke`, and `locomo-quickstart-bil2`. The branch now includes the advisory scheduled/manual repository-maintenance workflow (`.github/workflows/repository-maintenance.yml`) plus `tools/ci/github_maintenance_report.py` and tests to report open PRs, stale PRs, and stale remote branches without preserving provider session links in report artifacts. PR #32 required checks pass; the broader `test-and-benchmark` matrix remains advisory and still fails on pre-existing CI dependency/platform issues. PR #31 is still a draft/conflicting stale-work candidate, and its generated assistant session-link trailer was redacted from the PR body during HISTORY#248. `benchmarks/external/locomo/adapters/seam.py` and `tests/audit/test_locomo_adapter_retrieval_event_writer.py` remain dirty with unrelated H2 retrieval-event writer-hook work and were not staged for repo hygiene. Step 4 final paid measurement remains operator-gated.
